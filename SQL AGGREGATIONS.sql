...
SELECT standard_amt_usd + gloss_amt_usd AS standard_and_gloss_total
FROM orders;
...
SELECT MIN(occurred_at) AS earliest_time
FROM orders;
...
SELECT TOP 1 occurred_at AS earliest_time
FROM orders
...
SELECT MAX(occurred_at) AS latest_time
FROM orders;
...
SELECT TOP 1 occurred_at AS latest_time
FROM orders
...
SELECT SUM(o.total_amt_usd) AS total_amt,a.name AS company_name
FROM orders o
INNER JOIN
accounts a
...
SELECT channel AS channel_name,COUNT(channel) AS time_channel_used
FROM web_events
GROUP BY channel;
...
SELECT s.name,w.channel,COUNT(w.occurred_at) AS number_of_occurrences
FROM sales_reps s
...
/* Write an SQL Query to retrieve the total sales for each channel from which clients access the company
Highlight value and low value clients */

SELECT w.channel AS Channel,SUM(o.total) AS Total_quantity,SUM(o.total_amt_usd) AS Total_sales_amount
FROM accounts a 
INNER JOIN orders o
ON a.id = o.account_id
INNER JOIN web_events w
...
/*
Write a query to calculate total sales and average order value by region
*/

SELECT r.name,
	SUM(o.total) AS total_sales_qty,
	SUM(o.total_amt_usd) AS total_amount,
	ROUND(AVG(o.total),2) AS average_order_qty,
	ROUND(AVG(o.total_amt_usd),2) AS average_order_amt,
	ROUND(SUM(o.total_amt_usd)/NULLIF(SUM(o.total),0),2) AS average_order_value
FROM region r
INNER JOIN sales_reps s
ON r.id = s.region_id
INNER JOIN accounts a 
...
/*AGGREGATING USING THE HAVING CLAUSE*/

/*
How many of the sales_reps have more than five accounts that they manage?
*/

SELECT s.name AS sales_rep_name, COUNT(*) AS number_of_accounts
FROM sales_reps s
INNER JOIN accounts a 
ON s.id = a.sales_rep_id
GROUP BY s.name
...
/*
Write an SQL query to calculate the average order value for each customer and filter those with an average above five
*/

SELECT a.name,ROUND(SUM(o.total_amt_usd)/NULLIF(SUM(o.total),0),2) AS average_order_value
FROM orders o
INNER JOIN accounts a
ON a.id = o.account_id
GROUP BY a.name
HAVING ROUND(SUM(o.total_amt_usd)/NULLIF(SUM(o.total),0),2) > 5
...
SELECT a.name AS account_name, COUNT(*) AS number_of_orders
FROM accounts a 
INNER JOIN orders o
ON a.id = o.account_id
GROUP BY a.name
...
--Leucadia National has the highest number of orders--
...
SELECT GETUTCDATE() AS curr_date_utc;

/*
Extract the month names from the occurreed-at column in the orders table.
*/

SELECT DATENAME(MONTH,occurred_at) AS Month_name, occurred_at
FROM orders;
...
SELECT DATEPART(MONTH,occurred_at) AS Month_number, DATENAME(MONTH,occurred_at) AS Month_name, occurred_at
FROM orders;

/* Deriving insights using the date functions */

/*
Find the sales in terms of amount for all orders in each year, ordered from greatest to least.
*/

SELECT SUM(total_amt_usd) AS orders_amount, YEAR(occurred_at) AS YEAR
FROM orders
GROUP BY YEAR(occurred_at)
ORDER BY orders_amount DESC;

/*
Which month did Parch and Posey have the greatest sales in terms of amount?
Are all months evenly represented by the dataset?
*/

SELECT SUM(total_amt_usd) AS total_amount,MONTH(occurred_at) AS MONTH
FROM orders
...
SELECT SUM(total_amt_usd) AS total_amount, DATENAME(MONTH, occurred_at) AS month_name 
FROM orders
...
-- It is obvious that 2013 has only the twelfth month(December) recorded in it --

SELECT DISTINCT MONTH(occurred_at) AS month_date
FROM orders 
WHERE YEAR(occurred_at) = 2017; 
-- It is obvious that 2017 has only the first month(January) recorded in it
...
-- Results show that walmart spent the most on gloss paper in march,2016 --
