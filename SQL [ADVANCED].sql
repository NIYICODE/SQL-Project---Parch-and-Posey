...
SELECT  a.name,a.primary_poc,s.name AS sales_rep
FROM accounts a 
...
/* If every $100 in purchase equates to 2 points and purchases above $1000 have a 2.0x points multiplier,
how many points will each customer have? */

WITH cus1 AS (
SELECT a.name, SUM(o.total) AS total_qty_sold,(o.total_amt_usd*2/100) AS points,
CASE WHEN (total_amt_usd*2/100) > 20 THEN 2.0 * (total_amt_usd*2/100)
ELSE (total_amt_usd*2/100)
END AS new_points
FROM orders o
...
SELECT name AS customer_name, ROUND(SUM(new_points),0) AS total_point_earned
FROM cus1
GROUP BY name
